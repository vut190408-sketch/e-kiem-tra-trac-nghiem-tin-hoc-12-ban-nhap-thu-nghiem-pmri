let QUIZ_DATA_SETS = {};
    ...
    const correctAnswerSound = null;
    const incorrectAnswerSound = null;
    ...
    try {
        ...
        QUIZ_DATA_SETS = await res.json();
        for (const topic in QUIZ_DATA_SETS) {
            const d = QUIZ_DATA_SETS[topic];
            if (d.version === "2.0") {
                const legacyArr = [];
                d.questions.forEach(q => {
                    let passage = '';
                    let mat = (d.materials || []).find(m => m.id === q.material_id);
                    if (mat) passage = mat.content;
                    let sectionName = (d.sections || []).find(s => s.id === q.section_id)?.title;
                    let lua_chon = [];
                    let dap_an_dung = [];
                    if (q.options) {
                        Object.keys(q.options).sort().forEach(k => {
                            const isCorrect = (q.correct_answers || []).includes(k);
                            const optText = q.options[k];
                            lua_chon.push((isCorrect ? '*' : '') + optText);
                            if (isCorrect) dap_an_dung.push(optText);
                        });
                    }
                    if (q.type === 'short_answer') dap_an_dung.push(...(q.correct_answers || []));
                    legacyArr.push({
                        cau_hoi: q.question, lua_chon, dap_an_dung, type: q.type, passage, groupId: q.material_id, section: sectionName
                    });
                });
                QUIZ_DATA_SETS[topic] = legacyArr;
            }
        }
        if (typeof populateQuizSelector === 'function') populateQuizSelector();
    } catch(e) {
        alert('Lỗi tải data.json!');
        console.error(e);
        return;
    }
        ...
        // =========================================================================
        const CURRENT_VERSION = 1785624393779;
